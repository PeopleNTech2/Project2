...
public class ConnectDB {
    ...
    public void insertToMongoDB() {
        ...
        document.append("name", "Anna");
        document.append("age", 26);
        document.append("location", "Hyderabad");
        //Inserting the document into the collection
        connectToMongoDB().getCollection("employees").insertOne(document);
        System.out.println("Document inserted successfully");
    }

    public List<String> readDataBaseMySql(String tableName, String columnName) throws Exception {
    ...
    public List<User> readUserFromMongoDB() {
        ...
        for (Document doc : iterable) {
            ...
            try {
                ...
                user.setAge(convertId);
            } catch (Exception ex) {
                idInt = (int) doc.get("id");
                user.setAge(idInt);
            }
            user = new User(user.getName(), user.getAge(), user.getLocation());
            list.add(user);
        }
        return list;
    }
    ...
    public static void main(String[] args) throws IOException, SQLException, ClassNotFoundException {
        ...
        int[] ids = {1, 2, 3, 4, 5};
        int[] numbers = {9, 4, 0, 7, 3};
        int[] new_numbers = {0, 10, 20, 30, 40};

//        connectDB.insertDataFromArrayToMySql(ids, "student", "ID");
//        connectDB.insertDataFromArrayToMySql(numbers, "student", "SortingNumbers");
//        connectDB.insertDataFromArrayToMySql(new_numbers, "student", "NewNumbers");
